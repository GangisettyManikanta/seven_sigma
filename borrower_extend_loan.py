...
class ExtendLoansScreen(Screen):
    ...
    def on_pre_enter(self):
        ...
        loan_extension = (float(loan_amount) * monthly_interest_rate * pow(1 + monthly_interest_rate, float(tenure))) / \
                         (pow(1 + monthly_interest_rate, float(tenure)) - 1)
        emi = loan_extension + float(loan_extension_months)
